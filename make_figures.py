...
cone_dynamics.py. Draws the following figure files (fig7_nusselt_ceiling and fig8_regime_map are drawn by
interface_bounds.py; see README.md for the mapping to manuscript numbering):
...
ax.plot(lam_c, fpp_c, 's', color='k', ms=7,
        label=fr'pre-projection estimate $\lambda\approx{lam_c:.3f}$')
ax.axvline(-0.46252, ls='--', color='tab:green', lw=1.0,
           label=r'projected $\lambda_c=-0.46252\pm0.00002$')
...
# Qualitative sketch only. Both profiles decrease monotonically from the wall,
# consistent with the positivity/monotonicity theorem of the manuscript.
fig, ax = plt.subplots(figsize=(5.2, 3.4))
eta = np.linspace(0, 6, 600)
th_noslip = 0.72*np.exp(-eta/0.16) + 0.28*np.exp(-eta/1.6)
th0 = 0.80
th_slip = th0*(0.62*np.exp(-eta/0.22) + 0.38*np.exp(-eta/1.6))
ax.axvspan(0, 0.6, color=(.9, .93, .98))
ax.axhline(1.0, ls=':', color='0.55', lw=0.8)
ax.plot(eta, th_noslip, '--', color='0.35', lw=1.3, label=r'no thermal slip: $\theta(0)=1$')
ax.plot(eta, th_slip, color='tab:blue', lw=1.7,
        label=r"thermal slip: $\theta(0)=1-L_2|\theta'(0)|<1$")
ax.plot([0], [1.0], 'o', color='0.35', ms=3.5)
ax.plot([0], [th0], 'o', color='tab:blue', ms=4)
ax.annotate('wall value depressed\nin proportion to the flux', xy=(0.0, th0),
            xytext=(1.2, 0.86), fontsize=7,
            arrowprops=dict(arrowstyle='->', lw=0.6))
ax.text(0.3, -0.10, 'inner layer', fontsize=7, ha='center')
ax.text(0.62, 0.52, 'thickness:\n' + r'$O(\varepsilon^{1/2})$ without suction' + '\n'
        + r'$O(\varepsilon)$ with suction $S>0$ (this work)', fontsize=7, ha='left')
ax.text(3.6, -0.10, 'outer region', fontsize=7, ha='center')
ax.set_xlabel(r'$\eta$'); ax.set_ylabel(r'$\theta$')
ax.set_title(r'Thermal boundary layer at high Pr, $\varepsilon=1/\mathrm{Pr}$ (schematic, not to scale)', fontsize=8.5)
ax.set_ylim(-0.17, 1.12); ax.set_xlim(-0.05, 6)
ax.set_xticks([]); ax.set_yticks([0, 1])
ax.legend(fontsize=7, loc='upper right', bbox_to_anchor=(1.0, 0.80), frameon=False)
fig.tight_layout()
save(fig, "fig6_slip_schematic")

# fig7_nusselt_ceiling is produced by interface_bounds.py, which adds the
# series-resistance lower bound of the manuscript.
