...
in: "Interfacial Thermal Resistance and Volumetric Heat Capacity Set Wall
Heat-Flux Bounds in an Aqueous Ternary Nanoparticle Dispersion".
...
It reproduces the tables and figure data of the manuscript (the residual table via
residual_diagnostics.py), including the direct-lambda march, the naive-a march and the
three-way boundary-estimate sensitivity comparison; README.md maps the internal table
and figure labels used below to the numbering of the submitted manuscript. It also reproduces the second-branch check of
...
Reproducibility code accompanying the above manuscript. See README.md for the
full author list and citation information.
...
def solve_scipy_bvp(P, einf, shooting_seed=None):
    ...
    return dict(ok=sol.status == 0, a=ya[2], b=ya[4],
                Cf=-P['A2']*ya[2], Nu=-(P['A5']+P['Rd'])*ya[4], finf=yb[0])
...
def table4_branch(P):
    ...
    log(f"  {0:7.3f} {u[0]:11.6f} {u[1]:11.6f} {s0['finf']:11.6f} "
        f"{-u[2]:11.6f} {-(P['A5']+P['Rd'])*u[2]:11.6f}")
    ...
    for k in range(1, 401):
        ...
        if k % 10 == 0 or o['y'][0] <= 0:
            log(f"  {s:7.3f} {v[0]:11.6f} {v[1]:11.6f} {o['y'][0]:11.6f} "
                f"{-v[2]:11.6f} {-(P['A5']+P['Rd'])*v[2]:11.6f}")
        if o['y'][0] <= 0:
...
def table5_prlimit(P):
    ...
    log(f"  Nu_inf = (A5+Rd)/L2 = {Nuinf:.6f}   gradient ceiling 1/L2 = {1/P['L2']:.6f}\n")
