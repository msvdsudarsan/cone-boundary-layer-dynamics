...
"Interfacial Thermal Resistance and Volumetric Heat Capacity Set Wall
Heat-Flux Bounds in an Aqueous Ternary Nanoparticle Dispersion": the generality
...
Reproducibility code accompanying the manuscript "Interfacial Thermal Resistance and Volumetric Heat Capacity Set Wall Heat-Flux Bounds in an Aqueous Ternary Nanoparticle Dispersion". See README.md for the full author list and citation
information.
...
# ---------------------------------------------------------------------------
# Part 5: Qc against suction at a second magnetic parameter (M=1.5), i.e. the
# last column of Table 6 of the code numbering (Table 8 of the manuscript)
# ---------------------------------------------------------------------------
def part5_qc_M15():
    from cone_dynamics import baseline, ratios, solve_state
    log("\n---- PART 5: f_inf and Qc against S at M=1.5 ----")
    log("      S       f_inf         Qc")
    P0 = ratios(baseline())
    for i in range(15):
        S = -0.5 + 0.25*i
        P = dict(P0); P['M'] = 1.5; P['S'] = S
        st = solve_state(P, P['einf'], P['h'])
        if (not st['ok']) or st['finf'] <= 0:
            log(f"   {S:6.2f}   no solution with f_inf > 0")
            continue
        Qc = P['Pr']*P['A4']**2*st['finf']**2/(16*(P['A5']+P['Rd']))
        log(f"   {S:6.2f}   {st['finf']:9.6f}   {Qc:9.6f}")
